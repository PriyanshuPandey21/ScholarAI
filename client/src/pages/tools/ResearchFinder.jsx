...
export default function ResearchFinder() {
  ...
  const handleSearch = async (e) => {
    ...
    setPapers([]);
    try {
      const { data } = await api.get("/api/tools/papers", {
        params: { query, limit: 10 },
      });
      setPapers(data.papers);
      setTotal(data.total);
      toast.success(`Found ${data.total.toLocaleString()} papers`);
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="page-title flex items-center gap-3">
          <BookOpen size={28} /> Research Paper Finder
        </h1>
      ...
      {!loading && total > 0 && (
        <p className="text-sm text-gray-500">
          {total.toLocaleString()} total results · showing top 10
        </p>
      )}
      <AnimatePresence>
        <div className="space-y-4">
          {papers.map((paper, i) => (
            ...
            >
              <GlassCard
                animate={false}
                className="hover:border-primary-500/30 transition-colors"
              >
